...
//テーブル作成　
    $sql = "CREATE TABLE IF NOT EXISTS abc" //tb5はテーブル名
    ." ("
...
if(!empty($enum)&&$passcheck==$epass){//編集のときは、上記で編集対象のパスワードを取得している
    $id=$enum;
    $sql = 'SELECT * FROM abc WHERE id=:id '; //作成したテーブル名
    $stmt = $pdo->prepare($sql);                  // ←差し替えるパラメータを含めて記述したSQLを準備し、
    $stmt->bindParam(':id', $id, PDO::PARAM_INT); // ←その差し替えるパラメータの値を指定してから、
    $stmt->execute();                             // ←SQLを実行する。
    $results = $stmt->fetchAll(); 
...
if(!empty($name&&$comment)&& empty($enum2)){//ここで重要なのは$enum2。これによって編集かそうでないか判断
    if(empty($pass)){
        ...
        $sql = $pdo -> prepare("INSERT INTO abc (name, comment,pass,date )VALUES (:name, :comment,:pass,:date)"); //テーブル名は5table
...
if(!empty($enum2)){
    ...
    $sql = 'update abc set name=:name,comment=:comment where id=:id';//tb5はテーブル名
...
if(!empty($dnum)){
    ...
    if(empty($dpass)or $dpass!=$passcheck){
        ...
        $id = $dnum;
        $sql = 'delete from abc where id=:id'; //tb5はテーブル名
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        $stmt->execute();
